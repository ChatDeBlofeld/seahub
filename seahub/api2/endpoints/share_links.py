...
class ShareLinksBatch(APIView):
    ...
    def post(self, request):
        ...
        for i in range(share_link_num):
            ...
            if s_type == 'f':
                FileShare.objects.create_file_link(username, repo_id, path,
                                                   password, expire_date,
                                                   permission=perm, org_id=org_id)

            elif s_type == 'd':
                FileShare.objects.create_dir_link(username, repo_id, path,
                                                  password, expire_date,
                                                  permission=perm, org_id=org_id)
...
class ShareLinkRepoTags(APIView):
    ...
    def get(self, request, token):
        ...
        for repo_tag in repo_tags:
            ...
            tagged_files = [item for item in tagged_files if
                            item.get('parent_path') and
                            item.get('parent_path').startswith(share_link.path.rstrip('/'))]
