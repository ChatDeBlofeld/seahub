...
class SeafileAPI {
  ...
  createMultiShareLink(repoID, path, shareLinkNum, autoGenreatePassword, expirationTime, permissions) {
    const url = this.server + '/api/v2.1/share-links/batch/';
    let form = new FormData();
    form.append('path', path);
    form.append('repo_id', repoID);
    form.append('number', shareLinkNum);
    form.append('auto_generate_password', autoGenreatePassword);
